...
conn = pymysql.connect(host='localhost',user='root',password='980224',db='dd',port=3306,charset='utf8')
...
def get_info(url):
    ...
    if res.status_code==200:
        ...
        for info in infos:
            ...
            cursor.execute("insert into blog_test (name,author,data,price,rate,comments)values(%s,%s,%s,%s,%s,%s)",
                           (str(name),str(author),str(data),str(price),str(rate),str(comments)))     #按对应字段写入数据库
